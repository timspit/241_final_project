...
_\autoref{tab:allarmsmodel})_ presents the results of all treatments collected. Note that while our survey design included an approach to test all combinations of political ideology (`ideology`: liberal/conservative), moral foundation (`arm_story`; purity/fiarness), and directionality (`arm_level`; base/extension) we did not hypothesize that a strict factorial design would be valuable and therefore did not test the treatments as such. Instead, we hypothesized that specific moral foundations would resonate differently among participants of different ideologies, and therefore tested effect of changes in directionality (`arm_level`) within each of the four arms comprised of combinations of ideology and story.  

The results demonstrate that the extension of the Purity foundation story, when shown to the Conservative participants, may positively affect participant opinions of UBI. This finding fits our prior expectation, which motivated us to focus on this arm as we gathered additional data. The Conservative participants had both a lower baseline opinion of UBI, which means that upward movement was possible, and the Purity story was expected to resonate with Conservatives. Therefore, the Extension (positive) treatment of the Purity story would logically leave Conservatives who would otherwise, ideologically, perhaps be skeptical of UBI, feeling optimistic and focused on purity - making them more amendable to helping.  

Our Base implementation (negative) only minimal upward coefficient
Our hypothesis was that some of the negative/Base treatments could have heterogeneous effects depending on interpretation of UBI in the context
Liberals already started so high
Prior research indicated conservatives would be likely to respond to Purity


[^1]    
_We still collected more data for all arms to confirm that of design, Con + Pure was most impactful (see _  
...
<!-- NOTES: -->
<!-- Gender -->
<!-- - Gender gap still interesting - a significant baseline difference between genders  -->
<!-- Familiarity -->
...
_Given that we collected in waves, wanted to ensure the wave/day blocking did not introduce an effect on our results (see \autoref{tab:wavestratmodel})_  

_Our design was not factorial (see \autoref{tab:factorialmodel}), which should preobably be deleted_  
...
```{r model_arm_cov_baalaance, echo = FALSE, results='asis'}
