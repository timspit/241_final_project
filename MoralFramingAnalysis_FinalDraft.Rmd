...
Another study that played a key role in defining this experiment was a paper by Lene Aaroe, Michael Bang Petersen and Kevin Arceneaux on 'Why and How Individual Differences in Disgust Sensitivity Underlie Opposition to Immigration'. _@arceneaux_2017 _ The subjects are tested on their support for immigration after their disgust response is triggered. This study finds some causal factors that influence political attitudes outside of one's conscious awareness and confirms that leveraging framing as a treatment lever does produce an observable effect on subjects.   
...
### _We still collected more data for all arms to confirm that of design, Con + Pure was most impactful (see \autoref{tab:model2})_  
...
stargazer(factorial_levels_com$lm
          ...
          , font.size        = "small"
          , column.sep.width = "1pt"
          , label            = "tab:modelfactorial"
          , single.row       = TRUE)
```

```{r our_spec_long}

### LEFT OFF HERE ### - just think we need to isolate the stories???
our_spec_long_con = custom_lm_calcs(lm_in = lm(ubi_number ~ conservative * arm_story * arm_level
                                               , data = results_factorial_pure)
                                    , clusters_in = NA)

stargazer(our_spec_long_con$lm
          # , factorial_levels_ind$lm
          # , se = list(our_spec_long_con$se_robust
                      # , factorial_levels_ind$se_robust
          # )
          , type = stargazer_type, header = F
          , title            = "All Arms Factorial Specifications"
          , column.labels    = c("Levels Independent")
...
```{r}
stargazer(lm(arm_level ~ gender + age + urban + ideology, data = results_armlibfair)
          , lm(arm_level ~ gender + age + urban + ideology, data = results_armlibpure)
          , lm(arm_level ~ gender + age + urban + ideology, data = results_armconfair)
          , lm(arm_level ~ gender + age + urban + ideology, data = results_armconpure)
          , column.labels = c("Lib + Fair", "Lib + Pure"
                              , "Con + Fair", "Con + Pure")
          , report = ('v*c*sp')
          , type = "text")
