...
results_full = results_full_dedup  %>%
  ...
  mutate(arm = case_when(grepl('a', fc_b_1, ignore.case = TRUE) ~ "purity_base"
         ...
         , fairness_q4_relieved = case_when(grepl('a', fc_e_4, ignore.case = TRUE) ~ fc_e_4
                                          , TRUE ~ NA_character_)
         # Bin reaction vars
         , purity_q2_repulsed_bin = case_when(is.na(purity_q2_repulsed) ~ 0
                                              , purity_q2_repulsed %in% c("None at all", "A little") ~ 1
                                              , purity_q2_repulsed %in% c("A moderate amount", "A great deal", "A lot") ~ 2
                                              , TRUE ~ NA_real_) %>% factor()
         , purity_q4_relieved_bin = case_when(is.na(purity_q4_relieved) ~ 0
                                              , purity_q4_relieved %in% c("None at all", "A little") ~ 1
                                              , purity_q4_relieved %in% c("A moderate amount", "A great deal", "A lot") ~ 2
                                              , TRUE ~ NA_real_) %>% factor()
         , fairness_q2_pain_bin = case_when(is.na(fairness_q2_pain) ~ 0
                                            , fairness_q2_pain %in% c("None at all", "A little") ~ 1
                                            , fairness_q2_pain %in% c("A moderate amount", "A great deal", "A lot") ~ 2
                                            , TRUE ~ NA_real_) %>% factor()
         , fairness_q4_relieved_bin = case_when(is.na(fairness_q4_relieved) ~ 0
                                                , fairness_q4_relieved %in% c("None at all", "A little") ~ 1
                                                , fairness_q4_relieved %in% c("A moderate amount", "A great deal", "A lot") ~ 2
                                                , TRUE ~ NA_real_) %>% factor()
...
#Create levels for the recruitment day variable
levels=c("Tuesday1", "Sunday", "Monday", "Tuesday2")
results_armlibfair$recruitment_day = factor(results_armlibfair$recruitment_day, levels=levels)
results_armlibpure$recruitment_day = factor(results_armlibpure$recruitment_day, levels=levels)
results_armconpur$recruitment_day = factor(results_armconpur$recruitment_day, levels=levels)
results_armconfair$recruitment_day = factor(results_armconfair$recruitment_day, levels=levels)
```

```{r}
#Review data for where we don't have apples to apples comparisons

#d = data.table(results_armlibfair)
#d = data.table(results_armlibpure)
#d = data.table(results_armconfair)
d = data.table(results_armconpur)
d[, .(observations =.N), keyby = .(recruitment_day, arm)]
```

```{r}
#Remove observations that don't give us apples to apples comparisons (e.g. data in the control group from different periods)

#In the conservative care story arm, only keep the data collected for the first Tuesday
results_armconfair<-results_armconfair[results_armconfair$recruitment_day=='Tuesday1',]

#In the liberal purity story arm, only keep the data collected for the two Tuesday collections
results_armlibpure<-results_armlibpure[results_armlibpure$recruitment_day=='Tuesday1' | results_armlibpure$recruitment_day=='Tuesday2',]

#In the liberal care story arm, only keep the data collected for the two Tuesday collections
results_armlibfair<-results_armlibfair[results_armlibfair$recruitment_day=='Tuesday1' | results_armlibfair$recruitment_day=='Tuesday2',]
...
```{r arm_summary, echo = FALSE, fig.height = 4, fig.width = 4, fig.align="center", fig.cap="Count of Participants by Arm/Panel"}
arm_summary = results_clean %>%
  group_by(ideology_bin, arm) %>%
  summarise(Count = n())

knitr::kable(arm_summary)
```
...
```{r exploratory_reactions_bin, echo = FALSE, fig.height = 8, fig.width = 8, fig.align="center", fig.cap="Reactions (Bin)"}
results_response_bin = results_clean %>%
  ...
  mutate(count_total_cohort = sum(count)
         ...
         , reaction = case_when(value == 2 ~ "effected"
                                , value == 1 ~ "not_effected"
                                , TRUE ~ NA_character_)) %>%
...
library(survey)
dstrat <- svydesign(id=~1, strata=~recruitday, data=results_armconpur)
summary(svyglm(ubi_number ~ arm_level, design=dstrat))

```

```{r methodology_model_1, fig.height = 4, fig.width = 4, fig.align="center", fig.cap="\\label{fig:model_1}Model 1 - Basic, Factorial Design Specification", results='asis'}
model1_libfair = my_lm_calcs(lm_in = lm(ubi_number ~ arm_level+recruitment_day, data = results_armlibfair), clusters_in = NA)
model1_libpure = my_lm_calcs(lm_in = lm(ubi_number ~ arm_level+recruitment_day, data = results_armlibpure), clusters_in = NA)
...
```
NOTES:  
- Purity Extension to the COnservatives if significant at to 0.05 level  - key result


```{r methodology_model_1_gender, fig.height = 4, fig.width = 4, fig.align="center", fig.cap="\\label{fig:model_1}Model 1 - Gender Interaction, Factorial Design Specification", results='asis'}
...
```
NOTES:  
- Gender gap seems to have closed with this new data  
- Slightly significant effect with women in con_pure extension arm but we see a stronger effect in aggregate


```{r methodology_model_1_familiarity, fig.height = 4, fig.width = 4, fig.align="center", fig.cap="\\label{fig:model_1}Model 1 - UBI Familiarity Interaction, Factorial Design Specification", results='asis'}
...
```
Notes:  
- Being familiar with UBI makes liberals higher and conservatives lower at baseline  
- No treatment interactions are significant  
...
```{r methodology_model_2, fig.height = 4, fig.width = 4, fig.align="center", fig.cap="\\label{fig:model_1}Model 2 - Conservative Purity Arm Specification", results='asis'}
...
```
Notes:
- Interesting that feeling relieved in the extension was significant and positive, while notbeing relieved was negative (with much more noise)
