...
# Exploratory
model_panel1 = my_lm_calcs(lm_in = lm(ubi_number ~ arm, data = results_clean %>% filter(panel == 1)), clusters_in = NA)
model_panel2 = my_lm_calcs(lm_in = lm(ubi_number ~ arm, data = results_clean %>% filter(panel == 2)), clusters_in = NA)

stargazer(model_panel1$lm, model_panel2$lm
          , type = stargazer_type, header = F
          , se = list(model_panel1$se_robust, model_panel2$se_robust)
          , title = "Moral Foundations Prelim Regression Specifications"
          , column.labels = c("Panel 1", "Panel 2")
