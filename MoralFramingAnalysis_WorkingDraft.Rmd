...
hom_lib_mod<-all_data[ panel==1 & arm != 'care_base' & arm != 'care_extension', .(ubi=as.numeric(UBI_2), panel, arm), ][, lm(ubi~arm)]
hom_lib_mod<-apply_robust_estimates(hom_lib_mod, vcovHC(hom_lib_mod))
# conservatives - homeless story
hom_con_mod<-all_data[ panel==2 & arm != 'care_base' & arm != 'care_extension', .(ubi=as.numeric(UBI_2), panel, arm), ][, lm(ubi~arm)]
hom_con_mod<-apply_robust_estimates(hom_con_mod, vcovHC(hom_con_mod))

# liberals - jacket story
jac_lib_mod<-all_data[ panel==1 & arm != 'purity_base' & arm != 'purity_extension', .(ubi=as.numeric(UBI_2), panel, arm), ][, lm(ubi~arm)]
jac_lib_mod<-apply_robust_estimates(jac_lib_mod, vcovHC(jac_lib_mod))
# conservatives - jacket story
jac_con_mod<-all_data[ panel==2 & arm != 'purity_base' & arm != 'purity_extension', .(ubi=as.numeric(UBI_2), panel, arm), ][, lm(ubi~arm)]
